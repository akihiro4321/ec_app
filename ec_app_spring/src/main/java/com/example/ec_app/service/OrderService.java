...
public class OrderService {
    ...
    public OrderHistoryResponse getOrderHistory(final int userId) {
        List<Order> orders;
        final List<OrderItemDto> orderItems = orderRepository.selectOrderItemsByUserId(userId);
        if (!orderItems.isEmpty()) {
            final List<Integer> orderIds = orderItems.stream()
                    .map(OrderItemDto::getOrderId).collect(Collectors.toList());
            final List<OrderDetailDto> orderDetails = orderRepository.selectOrderDetails(orderIds);
